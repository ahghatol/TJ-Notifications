...
use Joomla\CMS\MVC\Model\ModelList;
...
class Notifications
{
	 ...
	 * @since 1.0
	 */
	public static function send($adapter, $client, $key, $recipients, $replacements, $options)
	{
		switch($adapter)
		{
			case 'email':
				return self::email($client, $key, $recipients, $replacements, $options);
			break;
			...
			case 'push_notification':
				return self::pushNotificationios();
			break;
		}
	}

	public static function email($client, $key, $recipients, $replacements, $options)
	{
		try
		{
			// ListModel::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_notifications/models', 'NotificationsModel');
			// $model = ListModel::getInstance('Notifications', 'NotificationsModel', array('ignore_request' => true));
			//var_dump($model);die;
			require_once JPATH_ADMINISTRATOR . '/components/com_notifications/models/notifications.php';
			$model = ListModel::getInstance('Notifications', 'NotificationsModel', array('ignore_request' => true));	
	...
	public static function pushNotificationios()
	{
	}
	...
	{
		require_once JPATH_SITE . '/components/com_notifications/models/preferences.php';
		$model = ListModel::getInstance('Preferences', 'NotificationsModel', array('ignore_request' => true));
