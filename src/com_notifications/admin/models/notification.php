...
JModelLegacy::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_notifications/models');
 ...
 * @since  1.6
 */
class NotificationsModelNotification extends JModelAdmin
{
	/**
	 * Constructor.
	 *
	...
	{
		$config['event_after_save'] = 'nftOnAfterSaveNotificationTemplate';
	 ...
	 * @since    1.6
	 */
	public function getTable($type='Notification',$prefix='notificationTable',$config=array())
	{
		// Get the table.
	...
	public function getForm($data = array(), $loadData = true)
	{
		// Get the form.
		$form = $this->loadForm(
			'com_notifications.notification',
			'notification',
			array
			(
				'control' => 'jform',
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState(
			'com_notifications.edit.notifications.data',
			array()
		);
	...
	public function createTemplates($templates)
	{
		...
		$model       = JModelList::getInstance('Notifications', 'NotificationsModel');
	...
	public function delete(&$cid)
	{
		...
		foreach ($cid as $id)
		{
			...
			if ($data->core == 0)
			{
				...
				$deleteQuery->delete($db->quoteName('#__notification_user_exclusions'));
				...
				if ($result)
				{
					...
					$dispatcher->trigger('nftOnAfterDeleteNotificationTemplate', array($data));
				}
			}
			else
			{
	...
	public function getKeys($client)
	{
		...
		$query->from($db->quoteName('#__notification_templates'));
