 ...
 * @since  0.0.1
 */
class NotificationsControllerNotifications extends JControllerAdmin
{
/**
	* Proxy for getModel.
	*
	...
	* @since    1.6
	*/
	public function getModel($name = 'Notification', $prefix = 'NotificationsModel', $config = array('ignore_request' => true))
	{
	...
	public function delete()
	{
		...
		$msg['success'] = array();
		$msg['error']   = array();
		$modelDelete    = JModelList::getInstance('Notification', 'NotificationsModel');
		...
		foreach ($result as $res)
		{
			if ($res == 0)
			{
				$msg['error'] = JText::_('COM_NOTIFICATIONS_CORE_TEMPLATE_DELETE_MESSAGE');
			}

			if ($res == 1)
			{
				$count ++;
				$msg ['success'] = JText::sprintf('COM_NOTIFICATIONS_N_ITEMS_DELETED', $count);
			}
		}
	...
	public function enableEmailStatus()
	{
		...
		$model           = JModelAdmin::getInstance('Notification', 'NotificationsModel');
		$success         = 0;
		...
		foreach ($notificationIds as $notificationId)
		{
			...
			{
				$msg = JText::_('COM_NOTIFICATIONS_STATE_DISABLE_FAIL_MSG');
	...
	public function setState($value, $state)
	{
		...
		if ($success)
		{
			if ($state == 1)
			{
				$msg = JText::_('COM_NOTIFICATIONS_STATE_ENABLE_SUCCESS_MSG');
			}

			if ($state == 0)
			{
				$msg = JText::_('COM_NOTIFICATIONS_STATE_DISABLE_MSG');
			}
		}
		else
		{
