...
/**
 * Script file of Notification component
 *
 * @since  1.0.0
 **/
class Com_NotificationsInstallerScript
{
	 ...
	 * @return  void
	 *
	 * @Since notification version 1.0
	 */
	public function fixTemplateTable($db, $dbprefix, $config)
	{
		$query = "SHOW COLUMNS FROM #__notification_templates WHERE `Field` = 'title'";
		$db->setQuery($query);
		$check = $db->loadResult();

		if (!$check)
		{
			$query = " ALTER TABLE  `#__notification_templates` ADD  `title` varchar(100)	AFTER  `is_override`";
			$db->setQuery($query);
		...
		$query = "SHOW COLUMNS FROM #__notification_templates WHERE `Field` = 'user_control'";
		$db->setQuery($query);
		$check = $db->loadResult();

		if (!$check)
		{
			$query = " ALTER TABLE  `#__notification_templates` ADD  `user_control` int(1)	AFTER  `is_override`";
			$db->setQuery($query);
		...
		$query = "SHOW COLUMNS FROM #__notification_templates WHERE `Field` = 'core'";
		$db->setQuery($query);
		$check = $db->loadResult();

		if (!$check)
		{
			$query = " ALTER TABLE  `#__notification_templates` ADD  `core` tinyint(3)	AFTER  `user_control`";
			$db->setQuery($query);
		...
		$query = "SHOW COLUMNS FROM #__notification_templates WHERE `Field` = 'replacement_tags'";
		$db->setQuery($query);
		$check = $db->loadResult();

		if (!$check)
		{
			$query = " ALTER TABLE  `#__notification_templates` ADD  `replacement_tags` text	AFTER  `core`";
			$db->setQuery($query);
	...
	{
		$db = JFactory::getDbo();
		$link = 'index.php?option=com_notifications&view=notifications&extension=com_jticketing';
		$link1 = 'index.php?option=com_notifications&extension=com_tjvendors';
		$allLinks = '"' . $link . '","' . $link1 . '"';
