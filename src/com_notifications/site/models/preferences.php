 ...
 * @since  1.6
 */
class NotificationsModelPreferences extends JModelAdmin
{
	/**
	...
	public function getStates()
	{
		...
		$uid   = JFactory::getUser()->id;
			...
			$query->from($db->quoteName('#__notification_user_exclusions'));
	...
	public function save($data)
	{
		if ($data)
		{
			...
			$dispatcher->trigger('nftOnAfterUnsubscribeNotification', array($data));
	...
	public function deletePreference(&$data)
	{
		if ($data)
		{
			$dispatcher = JDispatcher::getInstance();
			JPluginHelper::importPlugin('notification');
			$dispatcher->trigger('nftOnAfterResubscribeNotification', array($data));
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$conditions = array(
			...
			$query->delete($db->quoteName('#__notification_user_exclusions'));
	...
	public function getTable($type ='Preferences', $prefix = 'NotificationTable', $config = array())
	{
