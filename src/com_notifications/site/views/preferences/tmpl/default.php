...
defined('_JEXEC') or die;
		...
		$language->load('com_notifications', JPATH_SITE, 'en-GB', true);
		$language->load('com_notifications', JPATH_SITE, null, true);
...
<script type="text/javascript">
	const nftBaseurl = "<?php echo JUri::root();?>";
	jQuery.noConflict();
	...
	function addPreferance(pId,client,provider,key)
	{
		if(pId)
		{
			jQuery.ajaxSetup({
				global: false,
				type:'post',
				url:nftBaseurl+'index.php?option=com_notifications&task=preferences.save',
				dataType:'json',
				beforeSend: function () {
					jQuery('#ajax-loader'+pId).show();
				jQuery('#ajax-loader'+pId).html("<img src='<?php echo JURI::root();?>components/com_notifications/images/ajax-loader.gif'><style='display:block'>");
				},
				complete: function () {
					jQuery('#ajax-loader'+pId).hide();
					jQuery('#tick'+pId).show();
					jQuery('#tick'+pId).html("<img src='<?php echo JURI::root();?>components/com_notifications/images/tick.png'><style='display:block'>");
	...
	function removePreferance(pId,client,provider,key)
	{
		 if(pId)
		 {
			jQuery.ajaxSetup({
				global: false,
				type:'post',
				url:nftBaseurl+'index.php?option=com_notifications&task=preferences.delete',
...
<form action="index.php?option=com_notifications&view=preferences" method="post" id="adminForm" name="adminForm">
	<div class="row-fluid">
		<div class="span6">
		</div>
	</div>
						...
						<?php foreach($this->keys[$menu->client] as $key=>$values) : ?>
							...
							<tr>
								...
								<?php foreach ($this->adminPreferences[$head->provider] as $adminKey => $admin) :?>
										...
										<?php if(empty($this->preferences)) :  ?>
											...
											<div class="control">
												<fieldset class="btn-group btn-group-yesno radio pull-left" >
													...
													<label class="btn-success" for="<?php echo $value.$i; ?>" ><?php echo JText::_('COM_NOTIFICATION_VIEWS_PREFERENCES_FIELD_ENABLE'); ?></label>
													<label class="btn" for="<?php echo $key.$i; ?>" ><?php echo JText::_('COM_NOTIFICATION_VIEWS_PREFERENCES_FIELD_DISABLE'); ?></label>
												</fieldset>
											</div>
										...
										<?php foreach ($this->preferences as $j => $prefer) :?>
											...
											<div class="control">
												<fieldset class="btn-group btn-group-yesno radio pull-left" >
														...
														<label class="btn" for="<?php echo $menu->client.$value.$i; ?>" ><?php echo JText::_('COM_NOTIFICATION_VIEWS_PREFERENCES_FIELD_ENABLE'); ?></label>
														<label class="btn-danger" for="<?php echo $menu->client.$key.$i; ?>" ><?php echo JText::_('COM_NOTIFICATION_VIEWS_PREFERENCES_FIELD_DISABLE'); ?></label>
												</fieldset>
											</div>
											<?php endif;?>
										<?php endforeach; ?>
											...
											<?php if ($count==0): ?>
												<div class="control">
													<fieldset class="btn-group btn-group-yesno radio pull-left" >
														...
														<label class="btn-success" for="<?php echo $menu->client.$value.$i; ?>" ><?php echo JText::_('COM_NOTIFICATION_VIEWS_PREFERENCES_FIELD_ENABLE'); ?></label>
														<label class="btn" for="<?php echo $menu->client.$key.$i; ?>" ><?php echo JText::_('COM_NOTIFICATION_VIEWS_PREFERENCES_FIELD_DISABLE'); ?></label>
													</fieldset>
												</div>
											<?php endif; ?>
									...
									<?php if ($temp == 0): ?>
										<span class="label label-warning"><?php echo JText::_('COM_NOTIFICATIONS_VIEW_PREFERENCES_TAB_UNSUBSCRIBED'); ?></span>
									<?php endif; ?>
